...
export default function RoundSetup({ allPlayers, matrix, settings, onSettingsChange }: Props) {
  ...
  const suggestTeams = () => {
    ...
    // Greedy: assign each player to the team where they've played least with current members
  ...
  return (
    <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
              <input
                ...
                style={{
                  ...
                  fontSize: "0.95rem", fontWeight: 600, fontFamily: "DM Sans, system-ui, sans-serif",
                  cursor: "pointer",
                }}
              />
              {existingRoundId ? (
            ...
            <div style={{ marginTop: "6px", fontSize: "0.82rem", opacity: 0.7 }}>
          ...
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            ...
            <button
              ...
              style={{
                ...
                opacity: (saving || totalAssigned === 0) ? 0.5 : 1,
                fontFamily: "DM Sans, system-ui, sans-serif",
              }}
            >
        ...
        <div style={{ display: "flex", gap: "12px", marginTop: "20px", flexWrap: "wrap" }}>
          {[
            { label: "Checked In", value: `${roster.length}/${allPlayers.length}`, color: "white" },
            { label: "Teams", value: teamsInUse, color: "white" },
            { label: "On Bench", value: unassigned.length, color: unassigned.length > 0 ? "#fbbf24" : "white" },
          ].map(stat => (
            <div key={stat.label} style={{
              background: "rgba(255,255,255,0.12)", borderRadius: "10px",
              padding: "10px 18px", minWidth: "90px", textAlign: "center",
              border: "1px solid rgba(255,255,255,0.15)",
            }}>
              <div style={{ fontSize: "1.4rem", fontWeight: 800, color: stat.color as string }}>{stat.value}</div>
              <div style={{ fontSize: "0.65rem", fontWeight: 600, opacity: 0.7, textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "2px" }}>{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        padding: "12px 24px", display: "flex", gap: "28px", flexWrap: "wrap", alignItems: "center",
      }}>
        ...
        {[
          ...
          <label key={s.key} style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer" }}>
            ...
            <span style={{ fontSize: "0.85rem", fontWeight: 500, color: "#374151" }}>{s.label}</span>
          </label>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: "0", minHeight: "600px" }}>
        ...
        <div style={{
          borderRight: `1px solid ${C.border}`, background: "white",
          ...
          <div style={{ padding: "16px 16px 10px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
              ...
              <span style={{ fontSize: "0.75rem", fontWeight: 600, color: C.navy }}>
                {roster.length}/{allPlayers.length}
              </span>
            </div>
        ...
        <div style={{ padding: "16px", background: C.bg }}>
          ...
          {roster.length > 0 && (
            <div style={{
              ...
              <span style={{ fontSize: "0.68rem", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em", marginRight: "4px" }}>
                Bench
              </span>
              {unassigned.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: "12px" }}>
            {Array.from({ length: maxTeams }, (_, i) => i + 1).map(num => {
              ...
              const combinedHC = teamPlayers.reduce((sum, p) => sum + (p.handicap_index ?? 0), 0);
                    ...
                    {teamPlayers.map(p => (
                      <div key={p.id} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                          ...
                          <button
                            onClick={() => assignToTeam(p, 0)}
                            style={{ background: "none", border: "none", color: "#d1d5db", cursor: "pointer", fontSize: "16px", lineHeight: 1, padding: "0 2px" }}
                          >
                            ×
                          </button>
                        </div>
                        ...
                        {unassigned.map(u => {
                          ...
                          return (
                            <option key={u.id} value={u.id}>
                              {u.display_name || u.full_name}{conflict > 0 ? ` (${conflict}x together)` : ""}
                            </option>
                          );
                        })}
                      </select>
