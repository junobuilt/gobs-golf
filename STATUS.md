...
<!-- SESSION admin-backup-pins-v2 (2026-09-05) — stacked newest-first; self-delimited so it merges branch-only alongside the others -->
**Last updated:** 2026-09-05 (Backup admin PINs v2 — named holders, calendar expiry, several at once; branch `admin-backup-pins-v2`, NOT merged)
**Session purpose:** Replace the H9 single-active-backup-PIN rule. **Several named people can now hold a backup admin PIN simultaneously**, each assigned to a player with its own calendar expiry (up to a year out), and the admin sees a list of exactly who has access and removes any one of them individually. **The single-active-credential rule is GONE** — minting no longer revokes or supersedes anything; that removal is the point of this version.
**Blocking bug found and fixed FIRST, alone (`50235c8`):** the backup-login path resolved only the NEWEST active credential (`.order(created_at desc).limit(1)`) and compared the entered PIN against it. Correct with one credential; with several, every holder except the most recently created one got "Incorrect PIN" — no log line, no way for the admin to diagnose. `resolveActiveBackupCredential` (`src/lib/backupCredentials.ts`, new SSOT) full-scrypt-verifies against EVERY active credential and binds the session cookie + audit row to the MATCHING one (no plaintext shortcut, no PIN index — both would defeat the pepper). **`src/middleware.ts` deliberately unchanged**: `backupCredentialLive(credId)` is scoped by the cookie's credential id and was already correct for N holders. **The real finding was the test gap** — nothing in the suite asserted WHICH credential a PIN resolves to, so had the fix been wrong, ALL existing tests would still have passed. `tests/lib/backupLogin.test.ts` closes it and is negative-controlled: restoring the v1 query fails 3 of its 6 tests (verified by reverting the source before committing).
**Migration 043 (`043_backup_pin_named_holders.sql`) — ADDITIVE, applied after a reviewed dry-run:** `admin_backup_pin.player_id` (bigint FK → `players`, ON DELETE SET NULL) + `holder_name text`, both **nullable so the one live pre-upgrade credential (prod id=7) survives untouched and keeps working**, plus a partial index on `player_id`. No backfill, no UPDATE, no DROP — 7 credential rows + 16 audit rows untouched. No unique constraint on `pin_hash` (salted, could never fire); no DB-level "one PIN per player" either (the predicate needs non-immutable `now()`) — both enforced in the action.
**Design point worth carrying forward:** **"Unnamed (pre-upgrade)" is branded off `holder_name IS NULL`, NOT `player_id IS NULL`.** The FK nulls `player_id` when a player is removed from the roster, so branding off it would relabel a real, named holder as "Unnamed" and leave the admin unable to tell who holds a live credential. `holder_name` is a name snapshot taken at mint time precisely so it outlives the player row. Both cases are tested (list layer + render layer).
**Also changed while in there (both deliberate, both tested):** (1) **`listBackupPins` throws instead of degrading to `[]`** — on a security screen "no holders" reads as "nobody can get in", and showing that when the read merely failed is the more dangerous lie; the card renders an explicit load-failure state. (2) The read is **bounded at 8s**, so a hanging request on poor course reception can't leave the admin staring at "…" forever on the one screen that answers "who can get into my app right now?".
**Expiry semantics:** resolves to **END OF DAY in America/Vancouver** (`endOfDayVancouverISO`, Intl-only, no dependency), so a PIN assigned for "August 29" works all day on August 29, and **a same-day PIN is possible** — which matters because the server runs UTC, where at 6pm Vancouver the date has already rolled and a naive check would reject it. DST-tested on both sides of both transitions. `formatLeagueDate` is the SSOT for the "August 29, 2027" copy shared by the list, the remove-confirmation, the reveal, and the server's rejection message.
**UI:** reuses `PlayerCombobox` (searchable, already sized for this audience) and `DangerModal` (the repo's locked dangerous-action pattern, confirming by name + date); native `<input type="date">` with min = **today** (not tomorrow) and max = one year out. Full names on this surface — it is a security list and the admin needs to know *which* Wayne.
**Green:** `tsc --noEmit` clean · **vitest 1443/1443** · **Playwright 73/73**.
**Playwright scope REDUCED — deliberate, not an oversight:** the spec asked for an assign→remove→login walk-through. It **cannot be driven by the current harness**: `installSupabaseMock` intercepts at the BROWSER layer, while every path in this feature is a server action or an Edge fetch that leaves the Next process and dies against the non-resolvable e2e sentinel host. It would need a **server-reachable mock** (a PostgREST-subset HTTP server + a different `NEXT_PUBLIC_SUPABASE_URL` for the dev server) — new infra that changes the env for all 73 specs, **NOT built on this branch**. The spec proves what it faithfully can (card in the admin gate, picker populated from the real intercepted roster query, date bounds, v1 controls gone, load-failure state), with the boundary documented in its header; the behavior itself is covered by vitest against the real code.
**Still open / NOT done (all deliberate):** **branch is NOT merged to master** — awaiting explicit approval. `supabase/schema.sql` **NOT regenerated** — `npm run db:backup` is PowerShell-only and there is no pwsh on the dev Mac; it was already stale from 2026-06-18 (missing 031–042) and 043 extends that to **031–043**; needs one `db:backup` run on a Windows machine (CLAUDE.md now records this). Server-reachable e2e mock (above). Type-safety hardening on the tournament match path (required `handicapAllowance`). Handicap-allowance admin UI incl. the 90% create-time four-ball default. Senior UX pass. Day 4 test-delete (session 39 / round 260 — **not executed**). S7 alternate-player swap verification. TD46 History batch-rewrite (~430→~12 round trips, left after S4's `loadRoundTeamsOnly`). TD47 tournament-nav shared cache across Home↔Tournament Home↔Scoreboard. TD45 cup early-clinch edge was named as parked, but **both its ROADMAP rows currently read ✅ FIXED** (planned-total/void accessor, 2026-08-04) — re-confirm against the live behavior before reopening it.

<!-- SESSION allowance-recompute-plumbing (2026-08-12) — stacked newest-first; self-delimited so it merges branch-only alongside the others -->
**Last updated:** 2026-08-12 (Allowance recompute-plumbing fix + create-time four-ball default; branch `fix/allowance-recompute-plumbing`)
**Session purpose:** Fix the Tournament 7 Day-2 divergence where match 90 (session 37, four-ball, `handicap_allowance = 90`) rendered **Canada 5 & 4** on the match **detail** page against a true **3 & 1** shown on the scoreboard. **Root cause:** `matchScorecard.ts` `buildMatchInput` built its `MatchInput` **without `handicapAllowance`**, so every live `recomputeState` ran the engine at the 100% default while the loader's `loaded.state` (via `assembleMatch`, correct) used 90%. The divergence was confined to the match detail page's **derived-outcome layer** (header/status margin, 18-hole review strip, "match over" banner, HoleDotRail tint, counting-ball arrow). **Every displayed number the admin read — `STROKES n` labels, per-player Net, stroke dots — came from `loaded` and was correct at 90% throughout; no displayed net was ever wrong.** Scoreboard, cup totals, standings, Tournament Home, and the standalone review page all read `loaded.state` and were unaffected. **No data loss, no stored results, cup total unchanged (9½–7½).** **The fix (branch-only, NO migration):** (1) surface `session.handicapAllowance` on `LoadedMatch.session` (`types.ts` + `loadMatch.ts` — value already fetched, just plumbed; no coalesce, `resolveTournamentAllowance` still owns the 100% default); (2) `buildMatchInput` now passes `handicapAllowance: loaded.session.handicapAllowance` into the engine — the whole behavioral fix, one call site (the only other hand-built `MatchInput` in prod, `assembleMatch`, was already correct); (3) **test gap closed** — `shotgunCrossSurface.test.ts` already asserted `recomputeState === loader.state` but its fixtures carried no allowance (both paths agreed at 100%); `makeLoaded` now takes an `allowance` (mirrors `assembleMatch` via `computeSideStrokes`), and two new cases guard it: a four-ball 90% board whose hole 5 (SI 5) flips (**verified RED without the fix, GREEN with it**) + a NULL≡100% case guarding Day 1/Day 3; (4) **create-time default DELIVERED** — `createSession` writes `handicap_allowance = 90` for `four_ball_match`, NULL for singles/greensomes (no backfill, no existing row touched). This **resolves the carry-forward** flagged by the 2026-08-10 session (a newly-created four-ball day was silently NULL⇒100%). **Verified against live match 90:** at 90% the fixed engine yields `side_b`, margin **3 & 1**, closeout **hole 10**, hole 2 = **side_a (USA)**, match strokes Terry 0 / Tom 10 / Ward 4 / Dan 7, `scoredBeyondCloseout = false` (holes 8–10 now count → the "extra scores" notice clears); the full hole-by-hole play-order table matches the spec exactly; at 100% it reproduces the old `5 & 4`. Read-only Supabase MCP SELECTs only — no writes. **Green: tsc clean · vitest 1408/1408 (was 1406, +2 parity) · Playwright 17/17 tournament specs (matchScorecard, dashboard, tournament, ClearHole, Hero, Landing).** **Committed branch-only — NO push/merge/deploy without Jonathan's explicit approval.**
**Still open / not done:** the allowance **admin UI** (per-session picker, edit-in-place, derived lock, "handicaps at 90%" caption) — unchanged from prior sessions; `editSession` deliberately does NOT set allowance on a format change (create-time only, per spec).
**FOLLOW-UP (own task — `schema.sql` drift, NOT just 042):** `supabase/schema.sql` is stale by the **ENTIRE tournament track — migrations 031–042 are all absent** (last regenerated 2026-06-18 at commit `9d97a43`, migrations 027/028, which predates `031_tournament_foundation`). So `tournament_sessions` (and every `tournament_*` table) does not appear in `schema.sql` at all — this is a 12-migration gap, not a one-column 042 lag. There is no `npm run db:schema`; the only regen path is the PowerShell `npm run db:backup` (interactive prod Session-Pooler credential prompt — not runnable from CC's bash). This branch ships **no migration**, so nothing here forced it; it was deliberately left OUT of the bugfix branch (regenerating would drag 12 unrelated migrations into it). **Action for Jonathan:** run `npm run db:backup` on a PowerShell machine with the prod connection string to fold 031–042 into `schema.sql` and commit the result as its own `chore(schema)` change. (Prior 040/041/042 sessions logged a narrower version of this carryover; the true scope is the full 031–042 track.)

<!-- SESSION admin-clear-hole (2026-08-11) — stacked newest-first; self-delimited so it merges branch-only alongside the others -->
**Last updated:** 2026-08-11 (Admin Clear hole + dot color + resume-hole fix; branch `admin-clear-hole`)
**Session purpose:** Ship, before Day 2 tee-off (Wed Aug 12), the fixes from two live incidents — an admin's stray `+` tap with no in-UI path back to blank, and a shotgun start-hole change that kept reopening the card on the old hole. **Four commits, branch-only, NO migration** (Commit 3 was rewritten to reuse the existing admin auth — the `players.is_admin` idea is cancelled). **1/4 resume-hole:** `pickInitialHole` gains `hasAnyScore` — a blank card ignores a stale saved hole and lands on the start hole (both callers updated; shared helper, not forked). **2/4 dot color:** `EntryRow` takes `side`, dots read `SIDE_TOKENS[side].base` (Canada dots red on the red card). **3/4 admin recognition:** extracted `backupCredentialLive` → `adminAuth.ts` (SSOT for middleware + route); new `GET /api/admin/status` runs the middleware's exact two checks, returns `{ isAdmin }` only, fail-closed, no-store; `useIsAdminSession()` fetch-once/load, default-closed, no polling. **4/4 Clear hole:** admin-only control in the entry row (ABSENT from the DOM for non-admins), gated on ADMIN not the scorer claim (clear never disturbs the claim), disabled on a finalised/locked round; explicit `ScoreOp:"clear"` → writers DELETE by key (never a 0), `overlayPending`+league overlay remove on a pending clear; `DangerModal` scope confirm + optimistic removal + toast; engine recomputes status/outcomes/nav/cup (reopens an auto-closed match). Threaded session `is_locked` → `LoadedMatch.session.isLocked`; added `danger-confirm`/`-cancel` testids to `DangerModal`. **Green: tsc clean · vitest 1406/1406 · Playwright 68/68.** Committed branch-only — NO push/merge yet (awaiting Jonathan's merge approval; target: merged + on production before Wed Aug 12).
**Tournament state (tid 7):** Day 1 (greensomes) PLAYED; Day 2 (four-ball) Wed Aug 12 at **90% allowance** (session 37 `handicap_allowance = 90`, set in prod); Day 3 (singles) Fri Aug 14 at **100%** (session 38 NULL). **Six Day-1 matches are still `status: pending` with no result posted.**
**Parked (NOT done):** senior UX pass on both apps; **Day 4 test-delete (session 39 / round 260 / 4 matches) — NOT executed** (deletion gate: needs Jonathan's explicit go-ahead); S6 write-queue durability close-out; S7 alternate-player swap verification; TD45 cup early-clinch edge; TD46 History batch-rewrite; TD47 tournament-nav shared cache.

<!-- SESSION tournament-handicap-allowance (2026-08-10) — stacked newest-first; self-delimited so it merges branch-only alongside the others -->
**Last updated:** 2026-08-10 (Tournament handicap allowance — engine + reader only; branch `tournament-handicap-allowance`)
**Session purpose:** **Make the tournament match-play engine allowance-aware (migration 042).** Plan-first, approved. Branch `tournament-handicap-allowance` off fresh `origin/master` (`e6aef0c`). **Scope this session = ENGINE + reader + types + threading + tests ONLY.** No admin picker, no scorecard caption, no backfill (those land in a follow-up session tomorrow with the edit-in-place control + the derived lock + the "handicaps at 90%" caption). **Migration 042** (`tournament_sessions.handicap_allowance integer NULL, CHECK 10–100`) was **authored + applied to prod by the planning chat (NOT CC)**; I committed the file `supabase/migrations/042_add_tournament_session_handicap_allowance.sql` verbatim to reconcile repo↔prod — did NOT author my own. **Decision A (load-bearing): NULL reads as 100%.** Prod check confirmed tournament 5 / session 26 (four_ball, round 241) has 66 scored rows — Option B (null⇒90%) would have retroactively rewritten those results, so we chose A; existing four-ball history + fixtures stay byte-identical. **Policy reversal (2026-08-10):** four-ball, long pinned to 100% as a deliberate departure from USGA 90%, is now a configurable per-session allowance; **singles stays 100% (USGA-correct, unchanged); greensomes stays 60/40 and ignores the allowance.** **SSOT (Amendment A):** the allowance is applied in ONE place — new helpers `resolveTournamentAllowance(format, sessionAllowance)` (per-format policy) + `tournamentPlayingHandicap(ch, allowance)` (math, delegates to league `getPlayingStrokes` so rounding matches league net) in `matchplay.ts`; BOTH call sites — the engine (`sideHoleNets`) and the loader/preview (`matchStrokes.ts` `computeSideStrokes`) — consume them, never a second multiply. **Threading:** `MatchInput.handicapAllowance?` (optional ⇒ pre-042 fixtures unchanged) + `TournamentSession.handicap_allowance` (required); `loadMatch.ts` `SessionRow` select adds the column and passes it to `matchInput` + `computeSideStrokes`; `PairingsPanel` GroupBuilder pre-save strokes preview reads `session.handicap_allowance` so preview == scored card. **Amendment B:** rewrote the stale `matchplay.ts:120` "four-ball 100% is deliberate, don't change" comment to record the reversal + date. **Tests:** new `tests/lib/tournament/handicapAllowance.test.ts` (12: policy, math/rounding, four-ball outcome flip 100%→90%, singles ignores, computeSideStrokes parity incl. greensomes ignore); 6 fixture-typed test files + e2e `supabaseMock` default the new column. **⚠️ CARRY-FORWARD (must not be lost — the follow-up UI session's #1 job):** with Decision A, a NEWLY created four-ball session is NULL and therefore reads **100%, not 90%** — a trap for next year's cup. **The follow-up UI session MUST write `handicap_allowance = 90` at four-ball session CREATION time.** (Wednesday is covered without the UI: Jonathan sets session 37's allowance directly.) **Green gate: tsc clean · full vitest · tournament Playwright — see below.** **Committed branch-only; NO merge to master, preview deploy only, NO production promotion without Jonathan's explicit approval.** **⚠ Carryover (unchanged): `npm run db:backup` to fold 041 + 042 into `schema.sql`.**

<!-- SESSION s-tournament-day-void (2026-08-06) — stacked newest-first; self-delimited so it merges branch-only alongside the others -->
**Last updated:** 2026-08-06 (Reversible day-level Void — published-event safety net; branch `s-tournament-day-void`)
**Session purpose:** **Reversible day-level Void (migration 041) — the published-event mirror of the reversible match-Void (040).** Plan-first, approved. **Build complete + green; migration 041 was authored + applied to prod by the planning chat (NOT by CC) — I only committed the file `supabase/migrations/041_tournament_session_void.sql` to reconcile repo↔prod; NO deletion; NO engine change (goldens byte-identical).** Branch `s-tournament-day-void` off fresh master (`1b71150`, which carries `s-tournament-day-delete`); **committed branch-only, NOT merged/pushed (Jonathan merges).** Worked in the `/Users/jonyang/Projects/gobs-golf` worktree (that branch was already checked out there). **Why:** closes the one real mid-tournament break — an admin creates a published day (or a day is cancelled, weather) that has scores and must stop affecting the cup. Published-side Delete stays locked (can't destroy real scores mid-event); the safe tool is a reversible **Void** — set the whole day aside (out of the cup, greyed, non-scorable) without deleting anything; un-void restores it. Available regardless of publish state. **Migration 041:** `tournament_sessions.is_voided boolean NOT NULL DEFAULT false`. **Accounting = ONE voided-set predicate (SSOT):** new `isMatchExcluded(m) = m.match.is_voided || m.session.isVoided` (`src/lib/tournament/cup.ts`) — a match leaves the decidable pool if it OR its day is voided. `deriveCupBar` (`activeMatches`/`voidedCount`), the standings roll-up (`loadDashboard`), and a player's next match (`findPlayerMatch`) all read it, so `liveTotal = coalesce(planned_match_total, createdCount) − voidedCount` and the win/retain lines shift as a voided day's matches drop. **Orthogonal to match-void by construction** (a session flag, not a bulk match flip): un-voiding a day clears ONLY the session flag, so an individually-voided match stays voided — the key correctness case. **`isMatchExcluded` landed in `cup.ts`** (no import cycle — `findPlayerMatch`/cards/dashboard/match-page import it cleanly; verified cup.ts's transitive deps never import findPlayerMatch). **Data path:** `is_voided` on `TournamentSession` + threaded onto `LoadedMatch.session.isVoided` (`loadMatch.ts` select + assemble). **Mutations:** `voidSession`/`unvoidSession` (state flags, NOT deletions — deletion gate untouched). **UI:** admin day card "Void day"/"Un-void day" (`Tournament.tsx`) — Void gated by a **reversible** `DangerModal` (`cannotBeUndone={false}`), Un-void applies directly (mirrors match-void UX); voided day greys + "Voided" chip; player scoreboard/cards render voided-day matches inactive (`TournamentMatchCard`, dashboard) + the day header greys/chips; match scorecard shows a "This day was voided" banner, scoring disabled (`match/[matchId]/page.tsx`); admin Created/Voided header readout counts the voided-set so it agrees with `liveTotal`. **Tests (+ cases across 5 files):** `cup.test.ts` (predicate + day-void shifts liveTotal/lines + un-void restores + **orthogonality**), `loadDashboard.test.ts` (voided-day roll-up exclusion), `findPlayerMatch.test.ts` (skips voided-day match), `admin-tournament.test.tsx` (reversible modal → `voidSession` → chip + Un-void; Un-void restores), `tournament-match-scorecard.test.tsx` (day-void + match-void inert banners); e2e `supabaseMock` defaults `tournament_sessions.is_voided`. **Green: tsc clean · vitest 1377/1377 (goldens byte-identical) · tournament Playwright 11/11.** Files: `cup.ts`, `loadDashboard.ts`, `findPlayerMatch.ts`, `loadMatch.ts`, `mutations.ts`, `types.ts`, `admin/tabs/Tournament.tsx`, `components/tournament/TournamentMatchCard.tsx`, `tournament/dashboard/page.tsx`, `tournament/match/[matchId]/page.tsx`, `041_*.sql`, 5 test files + `matchFixture.ts` + 6 fixture-typed test files + e2e `supabaseMock.ts`, ROADMAP/STATUS/DATA_PROTECTION. **CLAUDE.md mobile-chat + deletion-gate rules already present (confirmed, no change).** **⚠ Carryover (unchanged, NOT a blocker): `npm run db:backup` to fold prior migrations + 041 into `schema.sql`.**

<!-- SESSION s-tournament-day-delete (2026-08-06) — stacked newest-first; self-delimited so it merges branch-only alongside the others -->
**Last updated:** 2026-08-06 (Self-serve "Delete Day" for unpublished tournaments; branch `s-tournament-day-delete`)
**Session purpose:** **Self-serve "Delete Day" for UNPUBLISHED tournaments (partial R3).** Plan-first; the spec was re-affirmed verbatim after I surfaced the "single transaction vs. no-migration" tension. **Build complete + green; NO migration, NO schema change, NO deletion of prod data, NO engine touch** (goldens untouched). Branch `s-tournament-day-delete` (== `origin/master` `c351879`, which carries last session's Void/DangerModal work); **committed branch-only, NOT merged/pushed to master (Jonathan merges).** Worked in the `/Users/jonyang/Projects/gobs-golf` worktree (that branch was already checked out there). **Problem (Dad, testing):** a test Day 4 added to an unpublished tournament with scores couldn't be removed — the day-card Delete blocked with "remove the scores first" (R3 guard) and there's no bulk-remove-scores UI, so a sandbox day was undeletable without backend surgery. **Fix — gate the destructive path by publish state.** **Unpublished (`is_published=false`):** the day Delete now works even with scores — a two-tap `DangerModal` names the exact scope ("deletes N matches and X scores from Y players"), Cancel is the safe default, "Delete permanently" cascades the whole day. **Published (`is_published=true`): UNCHANGED** — the "remove scores first" guard holds. **Mutation** `deleteSession(id, { allowScores? })` (`src/lib/tournament/mutations.ts`): the escape hatch re-verifies invariants server-side in `forceDeleteDayWithScores` — unpublished at mutation time (`TournamentPublishedError`), the round is tournament-owned `rounds.tournament_id IS NOT NULL` never a league round (`NotTournamentRoundError`), and no sibling session shares the round (`SharedRoundError`) — then **ONE atomic cascading `rounds` delete** (scoped `.eq("tournament_id", …)`) removes the day in FK order (scores ← round_players ← round; day_sides + matches ← session ← round, via `tournament_sessions.round_id ON DELETE CASCADE`, mig 032). This meets the spec's "one transaction / no half-deleted day" WITHOUT an RPC (one statement = fully succeeds or deletes nothing); the stale `SET NULL` comment in `deleteSession` was corrected. New `countDayScores` (`queries.ts`) feeds the modal copy; `Tournament.tsx` branches `openDelete`/`confirmDelete`/`DangerModal` on publish state. **Test double** `tests/components/fake-supabase.ts` extended: a plain `from("rounds").delete()` now cascades its subtree via a shared `cascadeRoundChildren` helper (also folds `tournament_day_sides`, which the older `delete_tournament_cascade` RPC mock predated). **Tests (+3 files):** `deleteSessionForce.test.ts` (7: full cascade + other-day-intact, published refuse, `{allowScores:false}` still throws `SessionHasScoresError`, league-round abort, shared-round abort, rollback-on-failure = nothing partially deleted); delete-day component tests in `admin-tournament.test.tsx` (scoped counts, Cancel no-op, Delete-permanently full cascade, published block); `deleteDayAccounting.test.ts` (real `loadDashboard`+`deriveCupBar` before/after → createdCount 4→2, cup total 4→2, toWin 2.5→1.5, removed matches gone, no orphan counting). **Still parked:** R4 (soft-delete + audit log) and any published-side delete path; the self-serve teardown button stays gated on those. **Green: tsc clean · vitest 1367/1367 · Playwright 62/62.** Files: `mutations.ts`, `queries.ts`, `admin/tabs/Tournament.tsx`, `fake-supabase.ts`, 3 test files, `tournamentRoundsReads.test.ts` (count 5→7), ROADMAP/STATUS/DATA_PROTECTION (R3 → 🟡). **CLAUDE.md mobile-chat + deletion-gate rules already present (no change needed).** **⚠ Carryover (unchanged, NOT a blocker — no schema change this session): `npm run db:backup` to fold prior migrations into `schema.sql`.**

**Last updated:** 2026-08-04 (Session A — Scorecard "Resume to my spot", both apps; branch `s-scorecard-resume` off master)
**Session purpose:** **Scorecard resume-to-spot (both scorecards).** Plan-first, approved. **Build complete + green; NO migration; NO deletion; NO engine touch.** Branch `s-scorecard-resume` off `origin/master` (`51e74e2`); **committed branch-only, NOT pushed (Jonathan merges).** **Problem (Dad, live):** checking the leaderboard mid-round then returning to the scorecard reopened on numeric **hole 1** — the player re-hunts their spot and can mis-key the current hole's score into hole 1 (doubly wrong for a shotgun group that started on 12). **Fix — two layers, both scorecards** (`src/app/round/[id]/scorecard/page.tsx` regular; `src/app/tournament/match/[matchId]/page.tsx` tournament): **(1) Primary — remember last-viewed hole.** New guarded localStorage module `src/lib/scorecard/holeMemory.ts` (mirrors `deviceMemory.ts`: `gobs:sc-hole:` namespace, `typeof window` guard, try/catch, validates 1..18). Persist current hole on every change; on mount, land on the saved hole if present. **Keyed per scorecard** so it never leaks across rounds / between groups on a shared admin phone — regular `round:<id>:team:<teamFilter ?? "all">`, tournament `tournament:match:<matchId>`. **(2) Fallback — first UNSCORED hole in PLAY ORDER.** New pure `src/lib/scorecard/resumeHole.ts` (`resolveResumeHole` walks `holePlayOrder(startHole)` — the SSOT rotation from `matchplay.ts`, wrapping 18→1 — returning the first unscored hole, else the last in play order; `pickInitialHole` composes saved-else-fallback). **Shotgun trap avoided by construction:** the regular app has no shotgun start (startHole=1 → numeric), the tournament walks from `match.start_hole` — a group starting on 12 that scored 12–15 resumes on **16**, never numeric 1. Tournament "scored" = any score present on the hole across the group (`groupHoleHasScore`). Writer gated (`hydratedRef` / `!loading`) so the transient default isn't persisted before the initial resolution. **Tests (+2 files, +19):** `tests/lib/scorecard/resumeHole.test.ts` (start=12 scored 12–15→16; start=1 numeric; all-scored→last-in-order; wrap past 18; `pickInitialHole` honors valid saved / falls back on absent+corrupt) and `tests/lib/scorecard/holeMemory.test.ts` (round-trip, no cross-key leak, out-of-range/corrupt rejected, namespacing). **Existing suites updated for the intentional mount-hole change** (fixtures that seed hole 1 pinned to hole 1 via `setSavedHole`, or navigate to the hole-1 nav dot in e2e; Sequence D now asserts the *real* resume — remount reopens on the last-viewed hole 3). **Green: tsc clean · vitest 1337/1337 · Playwright 60/60.** Files: `resumeHole.ts`, `holeMemory.ts`, both scorecard pages, 2 new unit tests + 5 touched vitest files + 2 touched e2e (`allowance`, `shambles`), ROADMAP/STATUS. **CLAUDE.md mobile-chat rule already present (no change needed).** **⚠ Carryover (unchanged, NOT a blocker — no schema change shipped): `npm run db:backup` to fold 031–039 (+ pending 027 → 028 → 030 relays).**

<!-- SESSION-B (s-tournament-total-void-preview) — stacked newest-first; merges alongside s-scorecard-resume -->
**Last updated:** 2026-08-04 (Session B — Tournament: match total + void + admin preview; branch `s-tournament-total-void-preview`)
**Session purpose:** **Change 2 (status doubling) + Change 3 (declared total + void SSOT, resolves TD45) + Change 3b (admin preview doorway).** Plan-first, approved (with the under-declared `barSize` clamp). **Build complete + green; NO migration applied this session — migration 040 was already on prod via MCP; committed the file `supabase/migrations/040_tournament_declared_total_void.sql` to reconcile repo↔prod. NO deletion; NO engine change (goldens byte-identical).** **Change 2:** the `go-to-your-match` box (`src/app/tournament/page.tsx`) printed `matchStatus().text` AND `.thruText` — both `"Not started"` for a thru-0 match → doubled; now renders once (appends `thruText` only when it differs). **Change 3 (SSOT):** new `src/lib/tournament/cupTotals.ts` — the sole source for `liveTotal = coalesce(planned_match_total, created) − voided`, `winLine = liveTotal/2+0.5`, `retainLine = liveTotal/2`, `barSize = max(coalesce(planned,created), nonVoidedCreated)` (under-declared clamp). `deriveCupBar` (`cup.ts`) is the one caller; `PointsBar` reads it (geometry from `barSize`, win/retain lines at their point values, voided slots greyed, minor ticks thinned >~32, handles 2–50). Voided matches excluded from the standings roll-up (`loadDashboard`) + a player's next match (`findPlayerMatch`), render inactive on cards/scoreboard, non-scorable banner on the match page. `cupOutcome` UNCHANGED (feeding `total = liveTotal` makes its existing `remaining = total − decided` count planned-but-uncreated matches → the clinch fix; `cup.test.ts` acceptance table byte-identical). Setup: "Planned match total" input (2–50) on create + editable header control showing "Planned: N · Created: M" (`admin/tabs/Tournament.tsx`, `mutations.ts`). Void: per-match "Void match"/"Un-void" in Edit-Group modal via `DangerModal` (`admin/components/PairingsPanel.tsx`) → `voidMatch`/`unvoidMatch`; a state flag, deletion gate untouched. **Change 3b:** `"use server"` `isAdminSession` (`src/app/tournament/adminPreview.ts`) reuses `verifySession` (fail-closed); `/tournament` + `/tournament/dashboard` relax `!is_published` ONLY for an admin (preview banner); home hero / bottom nav (`getTournamentMode`) stay player-only; doorway links added to the admin Tournament tab. **Tests:** new `cupTotals.test.ts`, `deriveCupBar` declared-total/void + cross-surface (d) threshold, `loadDashboard` void-exclusion, landing status-once + preview-gate, `adminPreview` fail-closed, e2e player-gate vs admin-preview split (landing + dashboard); e2e mock defaults the two new columns. **Green: tsc clean · vitest 1337/1337 · tournament Playwright green (14 specs across landing/dashboard/hero/tournament/matchScorecard/pairings).** Screenshots of the PointsBar at planned=32/created=8 and planned=32+1-voided shared with Jonathan. **Committed to the branch; push per Jonathan's workflow.** **⚠ Carryover (unchanged): `npm run db:backup` to fold prior migrations; note 040 is now on prod + in the repo, so the next backup folds it into `schema.sql`.**

**Last updated:** 2026-08-03 (INTEGRATION + DEPLOY — 5 tournament/perf branches merged to master `6844888`, deployed)
**Session purpose:** **Integration merge + deploy of today's 5 branches.** Created `integrate-2026-08-03` off `origin/master` (`1b80665`) and merged the 5 branches **one at a time, in order** — **s1-labels-status → s4-perf-loaders → s2-day1-results-edit → s3-shotgun-cap-ab → s5-tournament-review-redesign**. **Every conflict was docs-only** (`STATUS.md` / `ROADMAP.md`); **ZERO code conflicts** — all `src/`+`tests/` files auto-merged (`mutations.ts`, `PairingsPanel.tsx`, `results.ts`, `loadRoundsList.ts`, `loadDashboard.ts`, `loadMatch.ts`, `match/[matchId]/page.tsx`, and touched tests). Docs conflicts resolved by the standing rule: session headers stacked **newest-first**, the shared 2026-07-29 "Fix Spec 2" prior block deduped to a single copy (kept the with-hashes version), **no content lost**. **Green gate on the integrated branch: tsc clean · vitest 1318/1318 · Playwright 59/60** — the lone failure `tournamentLanding.spec.ts:60` (`tmatch-card-500`) is **PRE-EXISTING**, verified by re-running it on clean `origin/master` (`1b80665`) where it fails identically; NOT introduced by the merge. **NO migration added by any of the 5 branches** (verified `supabase/migrations/` diff master→integration = empty) → the `db:backup`/schema-regen carryover is NOT a deploy blocker. **Merged to master via fast-forward** (`origin/master` unmoved at `1b80665`; master → `6844888`) and **pushed** (`1b80665..6844888  master -> master`) — Vercel auto-deploy from master triggered. **No branches deleted; the pre-existing `stash@{0}` ("WIP on s2-day1-results-edit") left untouched.** **⚠ Carryover (unchanged, NOT a blocker — no schema change shipped): `npm run db:backup` to fold 031–039 (+ pending 027 → 028 → 030 relays).**

**Last updated:** 2026-08-03 (S5 Change 8 — Tournament 18-hole review redesign; branch `s5-tournament-review-redesign` off master)
**Session purpose:** **S5 Change 8 — Tournament 18-hole review redesign.** Plan-first, approved. **Build complete + green; NO migration; NO deletion; NO engine touch.** Branch `s5-tournament-review-redesign` cut off `origin/master` (`1b80665`); **committed branch-only, NOT pushed (Jonathan merges).** Reworked the tournament match scorecard's bottom "18-hole review" area (`src/app/tournament/match/[matchId]/`) onto the canonical hole-outcome source. **THREE parts:** **(1)** Removed the per-side "All 18 holes ⌄" chevron dropdown + its expanded `HoleStrip` from `TeamBlock` in `page.tsx` (dropped the now-dead `open` state, `outcomes`/`playOrder` props, `holePlayOrder`/`HoleStrip` imports, and the two now-unused `state`/`startHole` render-fn params) — the colored per-hole strip now lives ONCE. **(2)** In `MatchReviewGrid`'s "Hole results" strip, replaced the divergent hand-rolled `OutcomeStrip` (which used **A/B**, natural 1–18 order, and a different `getTeamColor(4/6)` palette than the scoreboard/nav) with the SHARED `HoleStrip` component fed `state.holeOutcomes` + `holePlayOrder(loaded.match.start_hole ?? 1)` — so the strip now reads the **exact same canonical per-hole winner** (`MatchState.holeOutcomes` from `computeMatchState`) that the hero `matchStatus()` + PointsBar read, in **play order** (shotgun start hole first), with the correct **A = USA (blue) / C = Canada (red) / ½ split / · unplayed** livery. Legend relabeled **B→C**, swatches recolored to `TOURNAMENT_TOKENS` so they match the strip cells. **NO second who-won computation anywhere** (SSOT hard requirement). **(3)** Per-player stroke grids (all 18 holes + F9/B9 via `PlayerHoleGrid`) left EXACTLY as-is. **Strip stays behind the "Review 18 holes" collapse** (per Jonathan — not always-visible; no all-hole info until that tap). **Shared `HoleStrip` change (additive, backward-compatible):** gained an optional `flaggedHoles?: number[]` prop (renders ⚑ above flagged cells — preserves the flag markers the read-only `/review` route relies on, since `/review` also consumes `MatchReviewGrid` and has no other flag surface) + per-cell `data-testid="hole-cell-{n}"`; **scoreboard callers pass neither → byte-identical render there** (verified: `tournamentDashboard` Playwright green). **Tests (`tournament-match-scorecard.test.tsx`):** the SSOT test repointed from the removed per-block chevron to the "Review 18 holes" section, now asserting the review strip's `hole-strip` textContent === the scoreboard `HoleStrip` === off `m.state.holeOutcomes`; review-grid assertions updated `review-outcome-N`→`hole-cell-N`, letter **B→C**, unresolved now renders `·`, legend "B = CANADA"→"C = CANADA". **Green: tsc clean · vitest 1297/1297 · Playwright green on affected specs (matchScorecard, tournamentDashboard, tournamentHero, tournament, pairings).** **⚠ The lone `tournamentLanding.spec.ts:60` Playwright failure (`tmatch-card-500` never renders) is PRE-EXISTING on clean `origin/master` — proven by stash-baseline (fails identically with my 4 files stashed) — NOT caused by this change; consistent with prior notes on tournament-landing flakiness.** Goldens byte-identical (no engine change). **Files:** `HoleStrip.tsx`, `MatchReviewGrid.tsx`, `match/[matchId]/page.tsx`, `tournament-match-scorecard.test.tsx` + ROADMAP/STATUS. **⚠ Carryover (unchanged): `npm run db:backup` to fold 031–039 (+ pending 027 → 028 → 030 relays); S2/S3 branch work still unmerged per prior sessions.**

**Last updated:** 2026-08-03 (S3 Change 4 — cap shotgun tee-off groups at A/B; branch `s3-shotgun-cap-ab` off master)
**Session purpose:** **S3 Change 4 — Cap shotgun tee-off groups at A/B.** Plan-first, approved (revised simpler scope). **Build complete + green; NO migration; NO deletion.** Branch `s3-shotgun-cap-ab` cut off `origin/master` (`1b80665`); **pushed branch-only, NOT merged to master.** **Problem:** in the admin shotgun setup the foursome "Group label" was a free-text `<input>` whose default/placeholder derived from `group_number` (`1→A, 2→B, 3→C, … 26→Z, 27→AA`), so a 3rd+ foursome could be labeled — and, even untouched, *displayed* — C, D, E, F… But **A/B is tee-off ORDER at a start hole** (Group A tees first, Group B second); a start hole holds at most two matches and **A/B are REUSED at every start hole** (multiple start holes each have their own A + B). The letter is display-only and does NOT feed team assignment or the scoring engine — confirmed: `matchplay.ts` reads `start_hole` (via `holePlayOrder`) for play order but never `group_label`; team/side comes from `side_a/b_team_number`. (Distinct from the hole-winner A/C system, Change 8 — not touched.) **Fix (two coordinated parts, app-layer only):** **(1) Picker** — the free-text input in BOTH `GroupBuilder` (Add) and `EditGroup` (Edit) in `PairingsPanel.tsx` is replaced with a two-button segmented **[ A | B ]** control (`GroupLabelPicker`, default **A**, 44px targets, navy-active, `role=group` aria-label "Group (tee-off order)"), storing exactly `"A"`/`"B"`; visible field label "Group (tee-off order)". Removed the now-dead `nextGroupNumber`/`nextGroupLabel` placeholder plumbing. **(2) Derivation** — `deriveGroupLabel` (`matchScorecard.ts`) is now `odd group_number → "A", even → "B"` (was base-26), so the fallback display for a 3rd+ group or a legacy NULL row shows **B, never C**. `EditGroup` seeds the picker from the effective letter (stored override → capped derive → "A") and writes the label only when it differs (an unrelated tee-only save never rewrites a NULL row). **Display left exactly as today** (letter still shows on non-shotgun groups, now A/B-capped) per the revised scope; plan item to hide it was DROPPED. **No DB constraint added.** **Data check:** all 16 prod `tournament_matches` rows have `group_label = NULL` — nothing to backfill; C–H never persisted, they were derived-only. `group_number` remains the unique foursome id (session 34 has 8 foursomes, unaffected); only the display label is capped. **Tests (+5):** `matchScorecard.test.ts` — `deriveGroupLabel` is odd→A/even→B and ∈{A,B} for n=1..30, empty for null/0, `groupLabelFor` honors an explicit A/B override else capped derive; `pairings-panel.test.tsx` — builder offers only `group-label-A`/`-B` (no `-C`, no old free-text `group-label`), defaults A, and round-trips "B" into the inserted row's `group_label`. **Green: tsc clean · vitest 1301/1301 · Playwright 6/6 (pairings, tournament, matchScorecard, dashboard).** **Staged my 4 files + ROADMAP/STATUS; left `.claude/settings.local.json` unstaged.** **⚠ Carryover (unchanged from the tournament line): `npm run db:backup` to fold 031–039 (+ pending 027 → 028 → 030 relays); S2 Change 3 (Day-1 greensomes hole edit) is on branch `s2-day1-results-edit`, still unmerged.**

**Last updated:** 2026-08-03 (S2 Change 3 — Day-1 greensomes hole-score edit in admin "Results & overrides"; branch `s2-day1-results-edit` off master)
**Session purpose:** **S2 Change 3 — Day-1 (greensomes) hole-score edit in "Results & overrides."** Plan-first, approved. Build complete + green; **NO migration; NO deletion.** Not yet pushed — awaiting Jonathan's go-ahead after diff review. **Gap:** the admin Results panel (`MatchOverridePanel.tsx`) offered per-hole score correction only for individual formats (Day 2 Best Ball / Day 3 Singles via `HoleCorrection` → `setMatchHoleScore` → `scores`); Day 1 greensomes got a static note deferring to the scorecard, because a greensomes hole is ONE team score on `team_scores` (ball_index 1), not per-player. **Fix (SSOT — no second scoring path):** new additive mutation **`setMatchTeamHoleScore(roundId, teamNumber, hole, strokes)`** (`src/lib/tournament/mutations.ts`) — the alternate-shot twin of `setMatchHoleScore`, same validation, writing through `upsertTeamScore` (the SAME `team_scores` upsert + 4-col conflict key `round_id,team_number,hole_number,ball_index` at ball_index 1 that the live Day-1 scorecard's team write queue uses). New **`GreensomesHoleCorrection`** control replaces the static note: picks a **SIDE (team)** not a player, + hole + strokes, and **PREFILLS the current team score** (syncs on side/hole change and on reload) so Dad sees the value he's changing. The match result + cup standings recompute canonically on the panel's reload — greensomes result is read-derived (`loadMatch`/`loadDashboard` → `team_scores` ball_index 1 → `computeMatchState` → `resolveMatchResult` → banked). `team_scores` + conflict key already exist → no migration. **Tests:** `tests/lib/tournament/overrides.test.ts` +3 (greensomes seed: team_scores upsert asserts the 4-col key + ball_index 1; `loadMatch` flips halved→side_a from the corrected team score; `loadDashboard` banked fold; validation); new `tests/components/match-override-greensomes.test.tsx` (control renders — not the old note; strokes input prefilled with the stored value; Save writes through the team_scores upsert). **Green: tsc clean · vitest 1302/1302 · Playwright tournament specs 6/6 (tournament, dashboard, matchScorecard, pairings).** Files: `mutations.ts`, `MatchOverridePanel.tsx`, `overrides.test.ts`, `match-override-greensomes.test.tsx`, ROADMAP/STATUS. **⚠ Carryover (unchanged): `npm run db:backup` to fold 031–039 (+ pending 027 → 028 → 030 relays).**

**Last updated:** 2026-08-03 (S4 — Performance: History nav teamsOnly projection + roster/holes hoist; tournament nav parallelize + name hoist; branch `s4-perf-loaders`)
**Session purpose:** **S4 — Performance (Changes 5 + 6).** Two fetch-path perf fixes on a new branch `s4-perf-loaders` off master — **build complete + green; NO migration; NO deletion; full `loadRoundResults` path byte-identical.** Root causes (read-only investigation, prior turn): both are Supabase fetch waterfalls (not render), but different mechanisms — **Change 6 (League "History" ~7s)** = `loadRoundsList` ran the FULL per-player `loadRoundResults` engine once per finalized round (**43 in prod** × ~10 reads ≈ ~430 round trips, throttled by the browser pool), discarding per-player detail the mini-leaderboard never renders + refetching the roster/holes per round; **Change 5 (Tournament Home / Scoreboard 2–3s)** = `loadDashboard` → per-day `loadSessionMatches` ran its 6–7 reads STRICTLY sequentially and refetched the tournament name once per day. **Change 6 fix:** added an ADDITIVE sibling `loadRoundTeamsOnly` (`src/lib/round/results.ts`) — a SEPARATE lightweight path that reuses the SAME total + rank SSOT helpers (`buildEnginePerTeam`/`individualTeamTotal`/`teamCardScalars`/`rankAndFormatTeams`) so team rank/name/roster/total/place equal the full path BY CONSTRUCTION, but skips all per-player detail (adjusted scores, stroke dots, per-player nets, Individual Rankings, blind-draw value recompute); and hoisted the round-INVARIANT roster + all-tees holes to ONE fetch for the whole list, injected per round (`src/lib/round/loadRoundsList.ts`). The full `loadRoundResults` is untouched. **Change 5 fix:** `loadSessionMatches` (`src/lib/tournament/loadMatch.ts`) now runs two concurrent waves (session-only reads together → round_players-dependent reads together), and `loadDashboard` threads the tournament (side names) — already held by all 3 call sites (`tournament/page`, `tournament/dashboard/page`, `TournamentHero`) — so the name is fetched ZERO extra times (optional/back-compat param; the 3 admin `loadSessionMatches` callers pass nothing → unchanged). **SSOT guard:** new `tests/lib/round/teamsOnly.test.ts` asserts teamsOnly === `loadRoundResults` per team (rank/name/roster/total/totalLabel/placeLabel/playerIds) across best-N / Stableford / Par Competition / Best Ball (net≠gross) + injected-vs-self-fetch equality. **Out of scope (logged as future TDs):** Change 6's full batch-rewrite (all rounds in a few chunked ≤1000-row queries — the ~430→~12 win) under TD46; Change 5's shared cache/prefetch across nav hops under TD47. **Docs:** ROADMAP TD45 (cup early-clinch, open from S1), **TD46 closed** (History perf), TD47 added (tournament-nav perf). **Green: tsc clean · vitest 1302/1302 (+6) · Playwright all touched specs pass EXCEPT the documented pre-existing `tournamentLanding.spec.ts:60` `tmatch-card-500` failure (proven pre-existing by stashing my changes → fails identically on the clean branch base).** Diff shown to Jonathan before commit; awaiting go-ahead to commit + push.

**Last updated:** 2026-08-03 (Tournament S1 — Labels & Status: Modified Alternate Shot label SSOT, "thru N holes", day tag COMPLETE + Cup P2 #4 reconciliation; branch `s1-labels-status`)
**Session purpose:** **Tournament S1 — Labels & Status (Changes 1, 7, 2).** Three display/status fixes on a new branch `s1-labels-status` off master — **build complete + green; ONE approved prod data UPDATE (not a deletion); NO migration; NO engine change** (27 tournament goldens + league goldenMaster byte-identical). Both tournaments are dormant/unpublished → no league-facing exposure. **Change 1 (Modified Alternate Shot everywhere):** the greensomes label was duplicated across **6** local `FORMAT_LABEL` maps (4 still read "Alternate Shot") + a match-page `FORMAT_SUB` — consolidated into ONE SSOT `src/lib/tournament/formatLabels.ts` (`greensomes: "Modified Alternate Shot"`) that every read/admin surface imports (`tournament/page`, `dashboard`, match page + `review`, `TournamentHero`, admin `Tournament`, `PairingsPanel`); sub-line → "Modified alternate shot · one score per team". **Absorbed parked P0 — verdict: the stored day/format CONFIG is NOT reverting** (`format=greensomes` correct on tid 5 AND tid 7). The persistent "Alternate Shot" came from the stored `tournament_sessions.name` ("Day 1 — Alternate Shot"), which surfaces render as `{name} — {FORMAT_LABEL}` → a DOUBLE-print, not a config bug. Fixed at source: `STANDARD_DAYS` seed names now bare ("Day 1"/"Day 2"/"Day 3") + **approved prod UPDATEs** — tid 7 sid 34 name `"Day 1 — Alternate Shot"` → `"Day 1"`, and the symmetric tid 5 fix (sid 25/26/27 `"Day 1 — Modified Alternate Shot"`/`"Day 2 — Best Ball"`/`"Day 3 — Singles"` → bare `"Day N"`). All 4 rows verified via RETURNING; no other tournament sessions embed a format in their name → no double-print remains anywhere. **Change 7 (thru N holes):** `matchStatus.thruText` + admin `dashboardFormat` emit "thru N **holes**" (hole-17 starters were misreading "thru 7"); single helper → hero + scoreboard by construction (cross-surface test in `tournament-match-scorecard.test.tsx` strengthened to assert the full string on BOTH). **Change 2 (day tag COMPLETE):** new SSOT `src/lib/tournament/completion.ts` — `isMatchComplete` / `isDayComplete` / `dayTag` (precedence COMPLETE > LIVE > UPCOMING). The day tags (Tournament Home + Scoreboard, **now every day**) AND the cup's `deriveCupBar` `decided`/`liveNow` read the SAME `isMatchComplete` predicate; tag styling shared via `DAY_TAG_STYLE` in `tokens.ts`. Also fixes "UPCOMING while already scored". Shipped wording "COMPLETE" (one-line swap to FINAL/DONE noted in the helper). **Cup P2 #4 reconciliation:** investigated prod — **NO phantom/duplicate matches** (tid 5: 8 across 3 days; tid 7: 8 Day-1 + Days 2–3 unpaired/0). The win-line count is clean. The genuine characteristic (dynamic `winLine = created-matches/2 + 0.5` can clinch the cup EARLY before later days are paired) is logged as **TD45**, NOT fixed (per Jonathan). **Tests:** +`completion.test.ts` (predicate + dayTag precedence + cup/day-tag agreement); updated thru/day-name/tag fixtures. **Green: tsc clean · vitest 1304/1304 · Playwright green on all touched specs.** ⚠ **`e2e/tournamentLanding.spec.ts:60` fails PRE-EXISTING** — proven by stashing S1 and re-running on clean master (fails identically; `tmatch-card-500` not visible). Unrelated to S1; NOT fixed this session. **Parked Change 6** (History ~7s global-nav load in the league app) → **TD46** for the S4 session. **Diff shown to Jonathan before commit; awaiting go-ahead to commit + push.** CLAUDE.md mobile-chat rule reconfirmed (dated 2026-08-03).

*Prior session (2026-07-29) — Fix Spec 2 — Scoreboard & Frontend (`b81b58f`/`868d1b3`/`8bec4fd`, branch `tournament`):* **Fix Spec 2 — Scoreboard & Frontend.** Five surface fixes on `tournament` — **build complete + green; NO migration; NO deletion.** Committed as three per-file commits (`b81b58f`, `868d1b3`, `8bec4fd`); **did NOT push (Jonathan merges).** **Item 1 (P1 — eager walk-off banner):** the "match over" banner fired on every stepper tap the instant the running total crossed the close-out margin, flickering while a 60–80 player was still adjusting strokes (read as "input locked"). The terminal banner (+ its scored-beyond / result-changed notes) now derives from a **SETTLED snapshot** of the optimistic scores — a ~900ms debounce (`settledScores`/`settledState`, resets on each tap) — so crossing-then-uncrossing mid-entry never shows it; it appears once the hole entry settles and the match is genuinely closed. The LIVE `state` still drives the status line + dots instantly, so input is never blocked; no engine change. **Item 2 (P2 — "You're You"):** the Tournament Home greeting fell back to the literal "you" when the stored player wasn't a match participant (the match-derived roster couldn't name them). `deviceMemory` now also persists the tapped display name (`getStoredPlayerName` / `setStoredPlayerId(id, name)`, cleared on switch); the greeting prefers it → roster lookup → name-less "You're all set" — never "You're you". **Item 3 (P2 — homepage order):** moved `<TournamentHero/>` BELOW the "Good Ole Boys" GOBS/admin card (was above); presentation-only reorder, hero still self-gates on tournament mode. **Item 4 (P2 — Home vs Scoreboard indistinguishable):** distinct hero eyebrows — Tournament Home "🏁 Tournament Home · Day N of M", Scoreboard "🏆 Scoreboard · Day N of M"; the homepage teaser hero keeps "🏁 Tournament · Day N of M". **Item 5 (P2 — shotgun start hole never "played"):** on a shotgun the card opens on the start hole as the CURRENT hole, whose gold-ring chip forced a white fill and masked the played color even after scoring; a scored current hole now wears its outcome color (blue/red/split) while keeping the gold ring, only an UNplayed current hole stays neutral white (`MatchHoleNav`). **Sole owner of `match/[matchId]/page.tsx` + `MatchHoleNav.tsx` + their tests this round (confirmed with Jonathan — A held Item B, C admin-only).** **Tests:** +2 in `tournament-match-scorecard.test.tsx` (banner absent on the deciding tap → present after settle; current/start hole played-color equals a non-current played hole ≠ an unplayed hole); the 3 edit-then-assert banner tests now `waitFor` the settle. **Green: tsc clean · vitest 1296/1296 · Playwright 9/9 (matchScorecard + tournament + hero + landing + dashboard).** **⚠ Parallel-session hygiene:** staged ONLY my files by name across the three commits (`deviceMemory.ts`, `tournament/page.tsx`, `tournament/dashboard/page.tsx`, `page.tsx`, `match/[matchId]/page.tsx`, `MatchHoleNav.tsx`, `tournament-match-scorecard.test.tsx`); left the concurrent admin session's + `.claude/settings.local.json` untouched. **⚠ Carryover: `npm run db:backup` to fold 031–039 (+ pending 027 → 028 → 030 relays).**
