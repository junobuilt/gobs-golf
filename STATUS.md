...
**Last updated:** 2026-07-28 (Tournament shotgun start + singles 1-on-1 split; branch `tournament`)
**Session purpose:** Two tournament-day features on `tournament`. **Build complete + green; migration 039 dry-run PASSED, NOT yet applied — awaiting owner go-ahead.** **F1 — Shotgun start:** migration **039** adds `start_hole` (1..18 CHECK, nullable) + `group_label` (text) to `tournament_matches` (per-group; both nullable → NULL start_hole = ordinary hole-1 start, NULL label = auto-derive A/B/C from group_number). The engine (`matchplay.ts`) is now **play-order + ORDER-AGNOSTIC**: `holePlayOrder(startHole)` (the shared rotation SSOT) drives thru / walk-off / margin, and completion no longer needs 1→18 order — a match counts when all holes are scored (any order) OR it is mathematically decided (walk-off, which counts unplayed holes incl. gaps as "remaining", so a forgotten hole can't cause a premature close). startHole=1 + no gaps ⇒ byte-identical to the old engine → **27 goldens unchanged**. The skip-hole amber is **retired** (`missingHoleGap` → always null; block removed). Scorecard nav (`MatchHoleNav`) renders rotated + wrapping; opens on the start hole. Admin (`PairingsPanel` builder + edit) sets start hole + label per group → `createGroup`/new `setGroupShotgun`. **F2 — Singles 1-on-1 split:** the match scorecard no longer co-renders a singles foursome's sibling — `loadGroup` returns `[target]` only, so each 1v1 is its own standalone 2-player card (reached via its own landing/dashboard row); `/review` already loaded a single match (no change needed). **Two load-bearing tests legitimately changed** (documented): matchplay §3.6 (order-agnostic: 8-14 now count, holesUp 2→5, still not decided) + the missing-hole amber test (retired). New shotgun engine tests + `shotgunCrossSurface.test.ts` (thru/margin agree between scorecard header and dashboard, both from one MatchState). **1259/1259 vitest (+10), tsc clean, tournament Playwright 10/10 green.** **⚠ NEXT: owner applies migration 039 via MCP (dry-run passed) → flip 039 header to APPLIED + `db:backup` fold 031–039.** **⚠ Unrelated: an uncommitted "Cup holder" change in `Tournament.tsx` + `MatchOverridePanel.tsx` was in the tree (not mine) — deliberately NOT staged/committed.**
...
Both features built + green; **migration 039 dry-run passed, NOT applied (awaiting owner go-ahead).** Feature 2 needs no migration. Nothing merged to master.
...
- **Migration 039** (`supabase/migrations/039_tournament_shotgun.sql`) — `ADD COLUMN start_hole integer CHECK (1..18), group_label text`. **Dry-run (BEGIN…ROLLBACK) run by me on prod `crscpwbuhvpiuxdebyxm`:** preflight (table present, both cols absent) → txn added both (nullable YES, CHECK `start_hole BETWEEN 1 AND 18`), 8 existing rows untouched (nullable add, no rewrite) → ROLLBACK → post-check both cols absent. **NOT applied — awaiting go-ahead;** on apply flip the file header to APPLIED + `db:backup` fold 031–039.
...
- **Owner: apply migration 039** via MCP (dry-run passed) → flip 039 header to APPLIED + `npm run db:backup` (folds 031–039; still-pending 027 → 028 → 030 relays in the same run).
