...
**Last updated:** 2026-07-27 (Tournament Phase 3.2 Relay A patch — nearest-match + roster fallback; branch `tournament`)
**Session purpose:** One-commit patch (`29caa2d`) on the Relay A device memory / landing. **Fix 1 — nearest match, not "today's":** the device-memory "go to your match" resolved the stored `player_id` only to today's date, so a future-dated tournament (July testing / opening the night before) blanked with "no match today" even when the player was assigned. New pure `findNearestMatchForPlayer(days, playerId, today)` — soonest day today-or-later; all-past → most recent (never blanks); null → genuine "you're in the tournament but not matched" note (kept). Link now names the day: "Your Day 1 match (Mon Aug 10) →". **Fix 2 — full-roster fallback:** "Who are you?" gains "Don't see your name? See all players" → lazily reads the full active league roster (inline `players` read, existing pattern — no new accessor) + the authoritative `tournament_players` set (existing `getTournamentPlayers`). A participant resolves normally; a league player NOT in the tournament → "You're not in this tournament yet — ask the admin to add you." (not stored, not added). **Membership = `tournament_players`, not "has a pairing"** — an assigned-but-unpaired player (Dad builds pairings the night before) is never wrongly told they're not in it. Both reads lazy (fallback-open only) → no extra load, `roundsQueryGuard` untouched. **1201/1201 vitest (+9), tsc clean, 57/57 Playwright.** **⚠ Carryover unchanged: relay migrations 027 → 028 → 030; `npm run db:backup` → TD38 (folds 031 + 032 + 033 + 034).**

*Prior session (2026-07-27):* Tournament Phase 3.2 Relay A — discovery + identity (5 commits) — see the section below.
