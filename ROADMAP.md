...
> **Phase B is fully shipped.** Math layer (5 formats, override engine, 133 unit tests, 4 snapshots), database foundation (B4.1–B4.5 ✅), format gate (B1.1–B1.6 ✅), and override / scoring-basis admin UI (B3.1–B3.3 ✅). Phase C (Leaderboard) is now unblocked.
...
> **Phase C follow-up:** round summary stroke totals currently display raw absolute (e.g. `37`), while scorecard pill shows delta (e.g. `−3`). Resolve uniformly during PR 2/3 leaderboard/drill-in work — recommended convention is delta everywhere, matching mockups.
...
| TD11 | Snapshot scripts need format filter guard in live-data regression loop | 📋 | Medium | `snapshot:b2/b3/b4/b5` Part 1 ("live-data 2-Ball regression") compare the engine's actual-format output against legacy 2-Ball best-2 math for **every** round in the database. Surfaced 2026-05-07: round 47 was played in a non-2-Ball format and produced 8 false-positive mismatches across all four snapshot scripts (same 8 every time). Every future non-2-Ball round in production data will compound the noise until fixed. Fix: add `if (round.format && round.format !== "2_ball") continue;` at the top of the Part 1 round loop in all four scripts so the legacy 2-Ball comparator only runs against 2-Ball (or null/legacy) rounds. Synthetic Part 2 cases stay untouched. |
...
| TD13 | `show_leaderboard` admin toggle now gates the wrong route | 📋 | Low | The Phase C, PR 2 route shuffle moved the per-player season stats page to `/season` and put a new live team leaderboard at `/leaderboard`. The `show_leaderboard` admin setting currently gates `/season` (since the page itself was relocated verbatim, gate logic and all). Label intent ("Show Leaderboard" / "Visible on the public leaderboard page") no longer matches what's gated. Either rename the setting to `show_season_stats` and add a separate gate for the live board, or apply the existing setting to both routes. Decide during admin settings polish pass — Settings tab also has TD10 (stale 2-ball toggle) and would benefit from one consolidated cleanup. |
...
- **Course Handicap (CH):** Slope-adjusted strokes received on a specific tee, stored in the `course_handicap` column of `player_course_handicaps`. Varies per tee. Round attribute (set via `computeCourseHandicap` when a tee is selected).
